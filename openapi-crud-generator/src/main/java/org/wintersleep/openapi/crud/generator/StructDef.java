...
public class StructDef {
    ...
    public StructDef(StructDefinition struct, Map<String, EnumDefinition> enumDefinitionMap) {
        ...
        if (struct.getProperties() != null) {
            for (var entry : struct.getProperties().entrySet()) {
                var propertyDef = PropertyDef.ofStruct(entry.getKey(), entry.getValue(), enumDefinitionMap);
    ...
    private Map<String, Schema<?>> generateProperties() {
        Preconditions.checkArgument(!properties.isEmpty(), "Property definitions cannot be empty for struct %s", name);
